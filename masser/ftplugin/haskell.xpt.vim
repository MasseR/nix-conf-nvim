...
XPT hasclass " class HasFoo .. ={}
class Has`Type~ a where
    {-# MINIMAL get`Type~, set`Type~ | `type~ #-}
    get`Type~ :: a -> `Type~
    get`Type~ = view `type~

    set`Type~ :: a -> `Type~ -> a
    set`Type~ = flip (set `type~)

    `type~ :: Lens' a `Type~
    `type~ = lens get`Type~ set`Type~
`cursor~

XPT hasinstance " instance HasFoo .. ={}
instance Has`Type~ `Super~ where
    `type~ = typed @`Type~
`cursor~
